  ...
  function printTopics($root, $level, $parentID){
    ...
    while($id = $IDResult->fetch_assoc()){
      ...
      while($topic = $topicResult->fetch_assoc()){
        $topicTitle = str_replace(" ", "",htmlspecialchars($topic['title']));

        if(strlen($topic['title'])<30){
          $titleExcerpt=htmlspecialchars($topic['title']);
        }else{
          $titleExcerpt = substr(htmlspecialchars($topic['title']),0,27)."...";
        }
  ...
  echo "\n<div class='mb-5 card h-100'>"; // Div to ensure padding around all modules/topics.
    ...
    echo "<div id='allModules' class='ml-1 w-100 card-body'>";
      while($module = $ModulesResult->fetch_assoc()){
        ...
        if(strlen($topic['title'])<30){
          $titleExcerpt = htmlspecialchars($module['title']);
        }else{
          $titleExcerpt = substr(htmlspecialchars($module['title']),0,27)."...";
        }
        $moduleTitle = str_replace(" ", "",htmlspecialchars($module['title']));
